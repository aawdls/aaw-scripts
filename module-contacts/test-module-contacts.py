...
def main():
    ...
    argument_parser.add_argument("-o","--output_filename", type=str, help="Path to output HTML file", nargs=1)
    ...
    # We assume that if a module exists in both, it has been moved to git
    ...
    startup_files = dict([line.split() for line in os.popen('configure-ioc list')])
...
"""
        ...
        output_file.write(list_template.page_head)
        output_file.write(list_template.body_start)
        output_file.write(list_template.table_head)
        ...
        for this_module in module_list:
            ...
            output_file.write(a_row % (this_module.module_name, this_module.vcs, contact, cc, number_of_IOCs))
            output_file.flush()

        logging.debug("Printing HTML footer")
        output_file.write(list_template.table_bottom)
        output_file.write(list_template.page_end)
